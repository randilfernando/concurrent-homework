...
double run_parallel(int n, double **mat1, double **mat2, double **mat_res);
...
void calculate_values(int n, int size) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        values_seq[i] = run_parallel(n, mat1, mat2, mat_res);
    }
...
#pragma omp parallel for shared(mat1, mat2, mat_res)
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            mat_res[i][j] = 0;
            for (int k = 0; k < n; ++k) {
